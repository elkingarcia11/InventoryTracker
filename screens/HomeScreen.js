...
class HomeScreen extends React.Component {
  ...
  createUser(s){
    var user = {name: s, total: 0, stock: 0};
    if(s.length > 0) {
      var newUserList = this.state.userList;
      newUserList.push(user);
      this.setState({userList: newUserList, modalVisible: false, name: ''});
    }
  }
  ...
  goToUserPage(ind){
    ...
    this.props.navigation.navigate('User', {
      name: this.state.userList[ind].name,
      total: this.state.userList[ind].total,
      stock: this.state.userList[ind].stock
    })
    console.log(this);
  }
    ...
    return (
      <Swipeout
        ...
        <ListItem
          ...
          onPress={()=>this.goToUserPage(item.index)}
          bottomDivider
          chevron
        />
      </Swipeout>
